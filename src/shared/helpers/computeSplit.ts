...
export function computeSplit({
  ...
  if (product_total_kobo < 0 || delivery_kobo < 0 || discount_kobo < 0) {
  ...
  const admin_cut_on_product = Math.round((product_amount_kobo * admin_percentage) / 100);

  const admin_amount_kobo = admin_cut_on_product + delivery_kobo;
